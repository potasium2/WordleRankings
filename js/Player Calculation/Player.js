class Player {
    ...
    DetermineRating(scorePosition, playerCount, guessCount, wordDifficulty, tagged = false) {
        ...
        const eloScaling = Math.min(Math.max(Math.pow(this.rating / 1000.0, 7.0), 1.0), 12.5);

        const scalingFactor = 5.0;
        const positionScalingFactor = Math.min(6, 4.0 * Math.max(0.85, 1000.0 / this.rating));
        
        const positionBonus = positionScalingFactor * (-Math.pow((10.0 / 3.0) * (scorePosition / playerCount), 0.825) + 5.0 / 3.0);
        ...
        this.rating += overallBonus - Math.max(1.0, eloScaling - 1);
