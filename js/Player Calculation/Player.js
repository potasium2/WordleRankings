class Player {
    ...
    DetermineRating(scorePosition, playerCount, guessCount, wordDifficulty, tagged = false) {
        ...
        const eloScaling = Math.min(Math.max(Math.pow(this.rating / 1000.0, 7.0), 1.0), 15.0);

        const scalingFactor = 4.0;

        const positionBonus = (-Math.pow(scalingFactor * (10.0 / 3.0) * (scorePosition / playerCount), 0.85)) + 7.5;
        ...
        overallBonus = overallBonus <= 0 ? Math.pow(scalingFactor, 1.5) * (positionBonus + guessBonus) : overallBonus;
